...
function setContract() {
  ...
  try {
    ...
    showResult(
      "contractResult",
      "Contract set successfully at address: " + address,
      "success",
    );
  } catch (error) {
    showResult(
      "contractResult",
      "Error setting contract: " + error.message,
      "error",
    );
  }
}
...
async function addCredential() {
  ...
  try {
    ...
    const tx = await contract.methods
      .addCredential(credIdBytes32, uri, validUntil)
      .send({ from: currentAccount });
    showResult(
      "addResult",
    ...
    showResult(
      "addResult",
      "Error adding credential: " + error.message,
      "error",
    );
  }
}
...
async function verifyCredential() {
  ...
  if (!didAddress || !credId) {
    showResult(
      "verifyResult",
      "Please enter DID address and credential ID",
      "error",
    );
    return;
  }
  ...
  if (!web3.utils.isAddress(didAddress)) {
    showResult(
      "verifyResult",
      "Please enter a valid Ethereum address for DID",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    const result = await contract.methods
      .verifyCredential(didAddress, credIdBytes32)
      .call();
...
async function revokeCredential() {
  ...
  try {
    const credIdBytes32 = web3.utils.keccak256(credId);
    const tx = await contract.methods
      .revokeCredential(credIdBytes32)
      .send({ from: currentAccount });
    showResult(
      "revokeResult",
